...
:)  
     if(  request:get-parameter("section", 'none')='level-data-combo' and $level > 0) then (
          let $data := browse-cache:get-cached-data( $browse:cache, $browse:LEVELS[ @pos  = $level ]/@vector, () )
 ...
 browse-summary:get($data, $browse:LEVELS[@pos = $level], $browse:URIs,  $browse:CATEGORIES, $browse:refresh-categories,  $browse:embeded-category-summary )


            if( $level = 3 ) then 
                browse-entries:titles-list( $data, $browse:LEVELS[ @pos  = $level ], $browse:URIs, $browse:CATEGORIES )   
      ...
      :)     
          ...
          let $data := browse-cache:get-cached-data( $browse:cache, $browse:LEVELS[ .  = 'entries' ]/@vector, '-grid'),
