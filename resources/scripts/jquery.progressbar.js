	...
	$.extend({
		progressBar: new function() {
			...
			this.defaults = {
				...
				boxImage		: 'progressbar.gif',						// boxImage : image around the progress bar
				barImage		: {
									0:	'progressbg_red.gif',
									30: 'progressbg_orange.gif',
									70: 'progressbg_green.gif'
								},
			...
			this.construct = function(arg1, arg2) {
				...
				return this.each(function(child) {
					...
					if (argvalue != null && this.bar != null && this.config != null) {
						...
						$bar.attr('src', config.imageDir + '/' + config.boxImage);
						...
						if (typeof(config.barImage) == 'object') {
							for (var i in config.barImage) {
								if (config.running_value >= parseInt(i)) {
									image = config.imageDir + '/' +  config.barImage[i];
								} else { break; }
							}
						}
						return image;
